...
def resource_path(relative_path):
    ...
    try:
        # PyInstaller 会创建一个临时文件夹并将路径存储在 _MEIPASS 中
...
def show_confession():
    # 强制 config.json 路径为当前目录
    config_file = resource_path('config.json')

    if not os.path.exists(config_file):
        win_message_box("错误", "配置文件 'config.json' 在程序目录下未找到！", MB_OK | MB_ICONINFORMATION)
        return
    ...
    # --- 简化音乐逻辑：直接查找名为 '1.mp3' 的打包音乐文件 ---
    # config.json 中的 music_file 路径将被忽略
    # 您必须将音乐文件命名为 1.mp3 (或在此处修改文件名)
    packed_music_file = resource_path('1.mp3')
    music_file_to_play = packed_music_file if os.path.exists(packed_music_file) else None
