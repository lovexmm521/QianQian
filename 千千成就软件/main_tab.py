...
class SelectLevelDialog(QDialog):
    def __init__(self, level_config, term, parent=None):
        super().__init__(parent)
        ...
        for level in level_config:
            self.level_combo.addItem(
                f"{level.get('level_name', f'等级 {level.get('level')}')} (需{term}: {level.get('wealth_threshold', 0):,})",
                userData=level)
...
class MainTab(QWidget):
    ...
    def open_level_select_dialog(self):
        if not self.level_config:
            ...
            return
        term = self.style_config.get("term_display_mode", "财富")
        dialog = SelectLevelDialog(self.level_config, term, self)
        if dialog.exec():
    ...
    def update_comparison_display(self):
        ...
        if needed > 0:
            self.comp_progress_label.setText(f"距离目标还需 {needed:,} {term}值")
        else:
    ...
    def update_display(self, total_wealth, current_level_data, next_level_data, sorted_config, current_level_index,
        ...
        self.wealth_value_label.setText(f"{term}: {total_wealth:,}")
        if current_level_data:
            ...
            if next_level_data:
                ...
                self.progress_label.setText(
                    f"距离下一级 [{next_level_data.get('level_name')}] 还需 {needed:,} {term}值")
            else:
            ...
            if sorted_config:
                ...
                self.progress_label.setText(
                    f"为达到第一个等级努力吧！还需 {first_level_wealth - total_wealth:,} {term}值")
            else:
