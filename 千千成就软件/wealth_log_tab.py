...
class WealthLogTab(QWidget):
    ...
    def apply_settings(self, style_config):
        ...
        self.current_term = style_config.get("term_display_mode", "财富")
        self.table.setHorizontalHeaderLabels(["日期", "当时等级", f"当前{self.current_term}值", "趋势", "说明"])
    ...
    def add_row_dialog(self):
        dialog = QDialog(self)
        dialog.setWindowTitle(f"添加{self.current_term}记录")
        ...
        form_layout.addWidget(QLabel(f"当前{self.current_term}值:"))
        ...
        if dialog.exec():
            try:
                ...
                QMessageBox.warning(self, "输入错误", f"当前{self.current_term}值必须是一个整数。")
    ...
    def export_data(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, f"导出{self.current_term}日志到 Excel", "", "Excel 文件 (*.xlsx)")
        if not file_path:
            return
        ...
        try:
            ...
            sheet.title = f"{self.current_term}日志"

            sheet["A1"] = "日期"
            sheet["B1"] = "当时等级"
            sheet["C1"] = f"当前{self.current_term}值"
            sheet["D1"] = "说明"
            ...
            QMessageBox.information(self, "成功", f"{self.current_term}日志已成功导出到:\n{file_path}")
    ...
    def import_data(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, f"从 Excel 导入{self.current_term}日志", "", "Excel 文件 (*.xlsx)")
        if not file_path:
            return
        ...
        try:
            ...
            self.save_log()
            QMessageBox.information(self, "成功", f"成功导入 {len(imported_logs)} 条{self.current_term}记录。")
